...
(def material (make-material (make-color 1 0.9 0.9) 0.1 0.9 0 200 (make-stripe-pattern white black)))
...
(def middle (set-transform (set-material (make-sphere)
                                         (make-material (make-color 0.1 1 0.5) 0.1 0.7 0.3 200 (make-stripe-pattern (make-color 0 0 1) (make-color 0 1 0))))
                           (translation -0.5 1 0.5)))

(def right (set-transform (set-material (make-sphere)
                                        (make-material (make-color 0.5 1 0.2) 0.1 0.7 0.3 200 (make-stripe-pattern (make-color 0 0 1) (make-color 0 1 0))))
                          (m*m (translation 1.5 0.5 -0.5) (scaling 0.5 0.5 0.5))))

(def left (set-transform (set-material (make-sphere)
                                       (make-material (make-color 1 0.8 0.1) 0.1 0.7 0.3 200 (make-stripe-pattern (make-color 0 0 1) (make-color 0 1 0))))
